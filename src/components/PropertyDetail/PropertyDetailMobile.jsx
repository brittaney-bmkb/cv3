...
const propertyDetailMobile = ({property1, property2, propertyColor1, propertyColor2}) => {
    ...
    const [ propertyStyle, setPropertyStyle ] = useState("left")
    ...
    useEffect(() => {
        ...
        propertiesToAdd.map((prop, index)=> {
            properStyleProps[index] = panelContentTitleMain
            properStyleProps[index]["borderColor"] = index === 0 ? propertyColor1: propertyColor2
            properStyleProps[index]["color"] = index === 0 ? propertyColor1: propertyColor2
        })

        setPropertyStyle(properStyleProps)
      }, [property1, property2]);
    ...
    useEffect(() => {
        if(screenWidth && screenWidth < theme.breakpoints.values.lg && properties.length > 1){
                ...
                <Box display="flex" flexDirection="row" columnGap={1} justifyContent={category === "top" ? "center" : textAlignment}>
                    {properties.map((property, propIndex) => {
                        ...
                        data.attributes['hyperlink_text'] && data.attributes['hyperlink_params'] && data.attributes['hyperlink_url'] ?
                            ...
                            justifyContent={textAlignment}
                            alignContent="center"
                            sx={index === 0 && category === 'top' && subIndex ===0 ? propertyStyle[propIndex]: null}
                            >
                                <Typography 
                                    align={textAlignment}
                                    variant="h5" 
                                    sx={{color: category === "top" && subIndex==0 ? propertyStyle[propIndex] : theme.main.text.dark }}>
